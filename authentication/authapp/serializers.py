...
from authapp.utils import Utils
...
class UserPaschangeSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if password !=password2:
           raise serializers.ValidationError("password does not matches with confirm password")
...
class UserSendEmailSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if User.objects.filter(email=email).exists():
            ...
            body='click to the following link to reset your password'+''+link
            data={
